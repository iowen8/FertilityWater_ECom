...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        public Nop.Services.Orders.PlaceOrderResult ProcessPaymentM(string paymentId, string paymentIntent, string paymentState, IList<ShoppingCartItem> cart, ProcessPaymentRequest processPaymentRequest)
        {
            var result = new Nop.Services.Orders.PlaceOrderResult();
            if (paymentState == "approved")
            {
                ...
                //using (var scope = new System.Transactions.TransactionScope())
                {
                    ...
                    if (!processPaymentRequest.IsRecurringPayment)
                    {
                        ...
                        //VAT number
                        var customerVatStatus = (VatNumberStatus)_workContext.CurrentCustomer.GetAttribute<int>(SystemCustomerAttributeNames.VatNumberStatusId);
                        if (_taxSettings.EuVatEnabled && customerVatStatus == VatNumberStatus.Valid)
                            vatNumber = _workContext.CurrentCustomer.GetAttribute<string>(SystemCustomerAttributeNames.VatNumber);
                    ...
                    var order = new Nop.Core.Domain.Orders.Order()
                    {
                        ...
                        OrderShippingInclTax = shippingOption.Rate,
                        OrderShippingExclTax = shippingOption.Rate,
                        ...
                        CardType = _encryptionService.EncryptText(processPaymentRequest.CreditCardType),
                        CardName = _encryptionService.EncryptText(processPaymentRequest.CreditCardName),
                        CardNumber = _encryptionService.EncryptText(processPaymentRequest.CreditCardNumber),
                        MaskedCreditCardNumber = _encryptionService.EncryptText(_paymentService.GetMaskedCreditCardNumber(processPaymentRequest.CreditCardNumber)),
                        CardCvv2 = _encryptionService.EncryptText(processPaymentRequest.CreditCardCvv2),
                        CardExpirationMonth = _encryptionService.EncryptText(processPaymentRequest.CreditCardExpireMonth.ToString()),
                        CardExpirationYear = _encryptionService.EncryptText(processPaymentRequest.CreditCardExpireYear.ToString()),
                        PaymentMethodSystemName = processPaymentRequest.PaymentMethodSystemName,
                        AuthorizationTransactionId = paymentId,
                        AuthorizationTransactionCode = paymentIntent,
                        AuthorizationTransactionResult = paymentState,
                    ...
                    if (!processPaymentRequest.IsRecurringPayment)
                    {
                        ...
                        foreach (var sc in cart)
                        {
                            ...
                            //attributes
                            string attributeDescription = _productAttributeFormatter.FormatAttributes(sc.Product, sc.AttributesXml, _workContext.CurrentCustomer);
                    ...
                    //reset checkout data
                    if (!processPaymentRequest.IsRecurringPayment)
                        _customerService.ResetCheckoutData(_workContext.CurrentCustomer, processPaymentRequest.StoreId, clearCouponCodes: true, clearCheckoutAttributes: true);
